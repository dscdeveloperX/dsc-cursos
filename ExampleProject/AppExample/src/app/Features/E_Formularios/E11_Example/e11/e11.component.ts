...
export class E11Component implements OnInit {
  ...
  public EnviarFormVideo(): void {
    if (this.formVideo.valid) {
      ...
      const categoriasSeleccionadas: ICategoriaVideo[] =
        this.categoriaVideoData.filter(
          (item) => this.formVideo.get(item.categoriaId)!.value === true
        );
